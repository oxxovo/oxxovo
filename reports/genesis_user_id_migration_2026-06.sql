...
--     레거시 NULL row끼리 충돌 없음(NULL은 서로 distinct지만 명시적으로 제외).
--     신규 신청(auth 필수, Phase 3)은 항상 user_id 세팅 → 1인1시즌1신청 강제.
--
-- 의도적으로 하지 않는 것:
...
--     Phase 5에서 reader 전수감사 후 (owner=auth.uid(), admin=is_admin(),
--     service-role bypass) 정책과 함께 ENABLE.
--
...
-- 3) 인덱스 확인 — 기대: 조회 인덱스 + 부분 유일 인덱스 2개 (+ 기존 인덱스/PK)
--    ※ 이 쿼리가 바로 '23505 핸들러' 미스터리의 정답: email 유일 인덱스가
--      여기 보이면 라이브, 안 보이면 23505는 방어/비활성 코드.
