...
4. **⑤ 변수는 이미 절반 배선돼 있다.** `LandingView.tsx`가 지금도
   ...
   본문에 넣게 한다 -- §4.
5. **④ 기간 가드용 날짜 -- 해소.** `getThemeRevealTime(season)`
   (`lib/seasons.ts:514`)을 그대로 호출한다. 11시간 차이났던 "11/8 12:00
   PT"는 본부가 준 값 자체의 오류였다(예선 결과 발표 시각과 주제 공개를
   혼동, HQ 2026-08-12 확인) -- 숫자를 고르는 게 아니라 함수가 정답이다.
   조사 과정은 §5에 남겨둔다.
...
1. ~~§5 -- 11/8 12:00 vs 23:00 PT~~ **해소(HQ 2026-08-12)**: 12:00 PT는
   본부가 준 값이었고 오류였다(예선 결과 발표 시각과 주제 공개를 혼동) --
   **어느 숫자도 고르지 않는다.** 가드는 `getThemeRevealTime(season)`을
   그대로 호출한다. 값이 바뀌면(시즌 일정 수정 등) 가드가 자동으로
   따라간다 -- 요건 ⑤의 정신 그대로. 위 §5 문단은 이 시점의 조사 기록으로
   남겨둔다(무엇을 왜 물었는지).
2. 제니3 금지어 목록 2종(경고용/기간차단용) -- **본부가 받아서 넘긴다,
   내가 직접 요청하지 않는다**(HQ 2026-08-12 지시).
