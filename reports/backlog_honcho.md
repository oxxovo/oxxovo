...
| 21 | ★정정(08-12 재확인): `app/apply/page.tsx`의 `MembershipGateScreen`은 studio 여부와 무관하게 먼저 렌더돼 **UI 퍼널은 이미 Studio 참가자도 gate한다.** 없는 건 그 화면 뒤(직접 `/studio` URL 접근, `registerForSeason`/submit 함수 자체)의 **서버측 재확인**뿐 — `checkApplyGate` 호출처가 여전히 `/api/apply` 1곳, `lib/studio.ts` 0곳인 건 실측 그대로 | 판정 필요 | 발견만, 손 안 댐(오늘 요청 범위 밖) | 미정 |
