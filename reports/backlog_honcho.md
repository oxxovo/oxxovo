...
| 21 | Studio 제출 경로에 `checkApplyGate`/Founding 클레임이 전혀 배선 안 돼 있음(2026-08-12 실측: `checkApplyGate` 호출처가 `/api/apply` 1곳뿐, `lib/studio.ts` 0곳, `MembershipGateScreen`도 `app/apply/page.tsx`에만) — season_0이 studio 전용이라 멤버십 필수 정책이 지금 이 경로에서 실효 0. `registerForSeason` 신설 시 이 결정을 새로 만들지 않고 있는 그대로 넘김(§⑤ 답변에 명시) | 판정 필요 | 발견만, 손 안 댐(오늘 요청 범위 밖) | 미정 |
