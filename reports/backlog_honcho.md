...
| 30 | Stripe 결제가 아직 test mode — `lib/stripe.ts isTestMode()`가 `STRIPE_SECRET_KEY`의 `sk_test_` 접두어만 본다(별도 플래그 없음). 라이브 전환 = (1) Vercel env `STRIPE_SECRET_KEY`를 `sk_live_...`로 교체 (2) `STRIPE_WEBHOOK_SECRET`도 같이 교체(test/live 웹훅은 별개 엔드포인트, Stripe 대시보드에서 라이브 웹훅 신규 생성 선행) — 가격은 `platform_config` 기반 inline `price_data`라 Price ID 이관 불요. 코드 변경 없음, 대표님의 Stripe 대시보드 자격증명이 있어야만 가능한 작업(2026-08-14 실측) | 대표님 작업(Stripe 대시보드+Vercel env 2건) | 없음 | **9/9 전** |
