...
- **(R0) 기준선 = "AI 생성물에 포함된 것은 OK, 후편집으로 추가한 것은 금지" [확정 2026-06-12]**: 분쟁 방지용 단일 판정 기준. AI 생성 클립 *안에* 들어 있는 글자·효과·전환·오디오는 모두 AI 생성물의 일부이므로 **허용**. 사람이 후편집으로 *추가*한 글자·효과·전환·외부 오디오는 **금지**. 모든 항목에 일관 적용:
  - AI 클립 안의 글자/효과/전환 = OK(생성 결과) / 후편집 추가 = 금지
  - AI 클립 자체 오디오 = OK / 외부 오디오 추가·믹싱 = 금지
  - **구조적 보장**: OXXOVO 편집기에는 글자·효과·전환·오디오를 *추가*하는 도구가 **아예 없음**(seq/trim/cut만). 따라서 후편집은 기능적으로 불가능 -- 규칙 위반이 UI 단에서 원천 차단됨. "AI가 생성한 그대로가 경쟁에 오른다."
  - **검증/채점 일관성**: CryptoBind EDL 검증(각 소스 클립 v1 서명 + EDL 서명 + 완성본 v1sc)과 채점 모두 이 기준선으로 판정. EDL은 seq/trim/cut 외 연산을 표현할 수 없으므로(jobId:startMs:endMs 튜플 나열뿐), 서명된 완성본 = 후편집 불가 증명. UI 카피(편집기 "왜 편집 기능이 제한되나요?" 펼침) + rules 페이지 §⑥ 헤드라인에 동일 문구 반영.
