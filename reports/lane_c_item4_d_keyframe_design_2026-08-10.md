...
★★**`toFfmpegExpr` 재측정 완료(같은 날 저녁, 제니2 지시) -- 정확하다.**
3점(`0→0.4→0.1`, 경계 t=1.5) 중첩 `if(lt(t,...))` 식을 실제 렌더하고, **경계
프레임(29·30·31, t=1.45/1.5/1.55 포함) 전부를 "같은 시점의 static 상수
`brightness=`"와 직접 대조**했다 -- 내가 손으로 만든 예측 공식(128+offset×255)이
아니라, ffmpeg 자신이 낸 static 값을 기준으로 삼았다(첫 시도에서 이 예측
공식으로 비교했더니 7~15 단위 어긋났는데, 원인은 표현식이 아니라 **내
예측 공식이 eq의 실제 밝기 매핑과 달랐던 것**이었다 -- static vs dynamic
직접 비교로 그 혼동을 없앴다). **결과: 7개 샘플 프레임 전부, 경계 프레임
포함, diff=0.00 -- 완전 일치.** 다중 제어점 중첩 `if` 문법은 그대로 쓸 수
있다. 설계 안 바뀐다.
...
결정 재확인: **2개**(①②, 정책)뿐 -- 엔지니어링 실측 항목(표현식 문법)도
오늘 저녁 해소됐다. 08-08의 "3+"에서 둘(워커 렌더 가능 여부=⑦, 표현식 문법
=오늘 저녁)이 빠졌다.
