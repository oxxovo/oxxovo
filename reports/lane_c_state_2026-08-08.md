...
| | HEAD |
|---|---|
| 앱 (`oxxovo`) | **`a1566cb`** (미커밋 0) |
| 워커 (`oxxovo-studio`) | **`52faee1`** (미커밋 0, 오늘 오후 이후 무변경) |

★**오늘 종료 시점 = ③b 전부 종료.** 격자 어휘가 저녁에 확정되어 `[0]`·`[4]`까지 끝났고,
이어서 ④가 이관되어 **E(split) 완료 · G(sharpen) 파리티까지** 갔다.

★**오늘 종료 = 프리뷰 배선 미착수, 그것이 노출의 유일한 블로커. 음악 마이그 대기분 4개는
배선만 남았다.**
...
## 5c. ★★파리티 "ALL PASS"가 무효 통과였다 — 정정과 유도 결과 (2026-08-08 저녁)

★**`test:parity:engine`의 ALL PASS를 근거로 쓴 곳을 정정했다**(제니2 지시):
`reports/lane_c_item4_pro_editor_survey_2026-08-08.md`(④ 판단)와
`reports/lane_c_item4_sizing_2026-08-08.md`(G 크기·결정 개수). 안 고치면 다음 사람이
같은 수치를 다시 근거로 쓴다.

**무동작(pass-through) 셰이더를 전 케이스에 돌린 실측** — 절대 게이트를 그냥 통과하는 조합:

| 고정 content 4종 | 실제 AI 프레임 2종 |
|---|---|
| **LUT 4/4** · glow/testsrc · **sharpen 4/4** = **9쌍** | **LUT 2/2** · **sharpen 2/2** = **4쌍** |

원인은 하나다: **게이트 값이 효과 자체의 크기보다 크다.** `color`는 정상(무동작 잔차
5~9% vs 2.5% → 전부 거부), `glow`도 실제 프레임에서는 정상(9.67·9.95%)이고 **testsrc에서만**
약했다(2.04%).
★**셰이더가 틀린 게 아니다** — LUT 상대 ratio 0.02~0.04(효과의 96~98% 재현). **게이트가 틀렸다.**

### ★게이트 형태 = 코드 조건으로 박았다 (`59eea1b`)

```
magnitude <= FLOOR(=1 LSB=0.392%)  → UNMEASURABLE  (통과 아님)
magnitude <  gate                  → RELATIVE      (ratio = 잔차/크기, 무동작=1.00)
그 외                              → ABSOLUTE      (기존 그대로, 재베이스라인 안 함)
```
효과별 수동 분류 없음 — **자동 분기**. 행마다 `[ABSOLUTE]`/`[RELATIVE]`/`[UNMEASURABLE]`을
찍고, 판정 불가는 **content별 이유**까지 출력한다. 무동작 대조군은 **코드로** 들어가 있다.

### ★★content 보강 결과 = **content가 문제가 아니었다**

제니2 승인으로 실제 AI 클립 프레임으로 다시 쟀다. **출처와 재현 방법**:
`reports/_shots/i2v_6shot_85fdc305/`의 `shot_mid_01.png`·`around6s_03.png` —
`reports/lane_c_i2v_6shot_realcall_2026-08-07.md`의 6샷 실호출(id `85fdc305-…`) 산출물.
★**이 파일들은 `.gitignore`의 `reports/_shots/`에 걸려 git에 없다.** 즉 "어디서든 재현"이
안 되므로 **고정 4종이 기본 content로 남고**, AI 프레임은 **출처를 붙인 추가 측정**이다.
재현은 그 mp4(역시 미커밋)에서 프레임 추출이거나 **유상 재호출**이다.

| | sharpen magnitude | ratio |
|---|---|---|
| mandel / testsrc (합성) | 0.72% / 0.71% | 0.26 / 0.21 |
| **실제 AI 프레임** | **0.64% / 0.53%** | **0.34 / 0.43** |

★**실제 영상도 바닥(0.39%)의 1.4배밖에 안 된다.** 정확한 셰이더의 하한
`FLOOR/magnitude` = **0.61~0.74**인데 무동작이 **1.00**이다 — **여전히 가를 값이 없다.**

### ★★★그래서 유도 결과 = **임계값이 아니라 계측 방식이 틀렸다**

sharpen은 **엣지 근처 픽셀만** 바꾼다. 그런데 지표는 **프레임 전체 평균 절대차**다 →
효과가 평균에 희석되어 **바닥 밑으로 내려간다.** content를 바꿔도 해결되지 않는 이유가 이것이다
(합성·실사 둘 다 0.5~0.7%).

---

## 5d. ★★★파리티 발견 전체 — **경로 순서대로** (결론만 받지 말 것)

★다음 사람에게 **결론이 아니라 경로**를 넘긴다. 7단계가 순서대로 서로를 만들었다.

**① 무동작 대조군 → "ALL PASS"가 무효 통과였다.**
`FRAG_NOOP`(pass-through 셰이더)을 전 케이스에 돌렸다. 절대 게이트를 **그냥 통과하는
조합이 9쌍**: **LUT 4/4 · glow/testsrc · sharpen 4/4**. 실사 프레임에서도 4쌍.
→ 근거로 쓰였던 문서 2건 정정(5b절 + ④ survey·sizing).

**② 원인 = 게이트가 효과 크기보다 컸다.**
게이트 3%·5%·2.5% vs 효과 자체 크기 1.34~2.61%·2.04%·0.18~0.72%. 무동작이 게이트 아래로
들어온다. ★**셰이더가 틀린 게 아니라 게이트가 틀렸다** — LUT 셰이더는 효과의 96~98%를
재현한다. 이 구분을 안 하면 멀쩡한 코드를 의심하며 시간을 쓴다.
★그리고 **"실패 시 exit 1"은 그 자체로 안전이 아니다** — 판별 못 하는 기준으로 exit 1을
걸면 **막는 척하는 게이트**가 된다.

**③ 계측: 신호상대오차 `Σ|gl−ff| / Σ|ff−plain|`.**
잔차를 **효과 자신의 픽셀별 신호**로 정규화. ★**평탄 영역이 분자·분모에서 함께 빠진다** —
그래서 희석이 사라진다. 무동작은 **구조적으로 정확히 1.000**(gl==plain이면 분자=분모).
★색보정이 절대 게이트로 잘 재진 이유도 같은 논리다: 색보정은 전 픽셀에 작용해 **평균이
효과를 대표**한다. 즉 게이트는 **효과의 공간 분포**에 따라 갈려야 한다.

**④ 3구간 + UNMEASURABLE.**
`PASS r<0.5 / REVIEW 0.5≤r<1.0 / FAIL r≥1.0`. ★**0.5는 측정값이 아니라 품질 기준**이다
("효과의 절반이 틀렸으면 그건 그 효과가 아니다"). **단단한 끝(1.000, 구조적)에서 유도**했고
무른 끝(최악 정상치 0.184)에 맞추지 않았다 — 그게 하드코딩이다. REVIEW 구간이 0.184~0.5의
빈 곳을 **조용히 통과시키지 않는다.**
★**바닥 규칙이 밴드보다 먼저다**: 효과 크기 ≤ 1 LSB(0.392%)면 **UNMEASURABLE**. 그 경우 r이
1.000으로 읽혀 무동작처럼 보이지만 실제로는 **content가 효과를 분해하지 못하는 것**이고,
FAIL로 부르면 **계측기 한계를 셰이더 탓으로** 돌린다. sharpen은 **smooth·bars가
UNMEASURABLE → 2/4만 판정 가능**.

**⑤ 커널 후보 6종 예측 적합 → 커널 무죄.**
조기 종료 조건 3개를 **첫 실행 전에** 적어두고 돌렸다. 결과: 1등(프로브 측정 2-D)이
**기존 binomial-5와 동률, gap 0.000**. 어떤 후보도 실사 0.338/0.433을 못 넘었다
→ **조건 ① 발동, 일곱 번째 후보를 만들지 않고 멈췄다.**
★진 후보 거리: box-3 0.476 / triangle-5 0.490 / binomial-3 0.536 / box-5 0.698.
★부수 측정: 프로브로 **ffmpeg 커널을 직접 뽑았고**(평탄 회색 + 밝은 픽셀 1개 → 블러가 곧
커널) **이항도 박스도 아니었다**. 다만 ⑤ 때문에 그 차이의 값이 작다.

**⑥ 범인 = 파이프라인 바닥.**
남은 두 축을 쟀다. ★**픽셀 포맷 무죄**(as-is == 강제 yuv444p, 바이트 동일 → 이미 올바른
포맷으로 협상). ★★**바닥이 범인**: unsharp **없이** RGB→yuv444p→RGB 왕복만 해도
**0.124~0.130%** 움직이는데 **효과 전체가 0.531~0.645%** → **환원 불가 양자화가 효과의
20~24%**.
★이 하나가 **모든 관측을 동시에 설명한다**: 커널 교체가 아무것도 안 바꾼 이유(바닥보다 작게
움직인다), 디테일이 많을수록 나빠진 이유(엣지 많음 = 양자화 사건 많음), 실사 0.433의 정체.

**⑦ 바닥은 차감이 아니라 옆에 표시(share).**
행마다 `floor|ratio|share`, **판정은 ratio만**. ★차감하면 **바닥과 셰이더 오차의 독립성을
가정**하는데 근거가 없다 — 떠도는 항이 진짜 오차를 상쇄해 지운다("뺄셈이 아니라 교집합").
바 조정도 안 한다(효과마다 바닥이 달라 하나로 못 정한다). 표본이 먼저다.

| share(바닥이 차지하는 비중) | color | LUT | glow | **sharpen** |
|---|---|---|---|---|
| 실사 | 14% | **254%** | 102~148% | **56~57%** |
| 합성 | 10~23% | 179~1832% | 26~1886% | ★**83~98%** |

★**share > 100%(LUT·glow)** = 오차가 파이프라인 자체 움직임보다 **작다** → 이 파이프라인이
표현할 수 있는 한계까지 정확하다. 절대 행이 늘 완벽해 보였던 이유.
★**color는 10~23%만 바닥** → 그 숫자는 진짜 셰이더 얘기고, **절대 게이트가 거기서만 통했다.**
★**sharpen은 대부분이 바닥** → 셰이더 몫 ≈ **0.19**(color 최악 0.184 옆). ⑤의 무죄가
**표에서 직접, 모든 content에서** 보인다. 커널 적합을 다시 돌릴 필요가 없다.

### 미확인 / 남긴 것

★`test:parity:transition`·`test:text-parity`·워커 `music-parity`에는 **같은 질문을 안 했다.**
전환 표의 "9종 worst ≤0.30%"도 같은 계열일 수 있다(**A 소관**).
★본부 규칙 ②의 소급 감사 결과 — 대조군 **있음**: `music-boundary`·`text-parity`·
`i2v-model-guard`·`round-cap-reset`. **목록(지금 고치지 않음)**: `music-grid-parity`
(자기비교 방지만) · `music-curation`(양방향은 있으나 무동작 개념 없음) · `landing-stage`.

### 도구 (전부 커밋됨 — 다음 패스는 회상이 아니라 측정에서 시작한다)

```
npm run test:parity:engine              밴드·바닥·무동작 대조군 전부 포함
node scripts/negctl-sharpen.mjs         효과 자체 크기 (amount 1.0 / 2.0)
node scripts/probe-unsharp-kernel.mjs   ffmpeg 커널 직접 추출
node scripts/fit-unsharp-kernel.mjs     후보 예측 적합 (조기 종료 조건 내장)
node scripts/probe-unsharp-axis.mjs     포맷·바닥 축 분리
```
...
★**남은 것은 `preview-gl.ts`에 `FRAG_UNSHARP`를 배선하는 것 하나이고, 그것이 노출의
유일한 실질 블로커다.** 셰이더는 파리티를 통과했고(밴드 PASS, 셰이더 몫 ≈ 0.19 ≈ color
0.184), 커널은 무죄이고, 게이트는 판별력을 갖췄다. **`EXPOSED_SLIDER_KEYS`는 아직 안
건드렸다** — 배선 없이 열면 렌더만 움직인다.

`preview-gl.ts`는 `FRAG_COLOR_LUT / BLUR / SCREEN / COPY / TRANSITION`만 프로그램으로
만든다(:47). 패스 1개 추가 = 프로그램 생성 + FBO 체인 안의 드로우 순서. ★**FBO 체인을
정확히 읽고 시작할 것 — 반쯤 하고 끊기는 게 가장 나쁘다.**

그다음 순서: **노출 → `chromatic`·`motionBlur`·`lutIntensity`(같은 방식, 효과당 독립 출하)
→ D 스파이크.** ★**D는 워커 값-곡선 스파이크가 선행**(`enable=`은 켜기/끄기이고 값 곡선이
아니다. `sendcmd`는 워커에 없다). ★**H·F는 발사 후·시즌0 범위 밖**(H는 길이 모델을 강제로
열고, F는 계약·음성 동의가 관문).

| 항목 | 소관 |
|---|---|
| ~~격자 축의 값~~ | ✅ **확정 2026-08-08** (제니3·본부) → [0]·[4] 완료 |
| `official_actors.slug='actor-3-beauty-cf'` 개명 | ★본체 (5b절) |
| 마이그 **음악 4컬럼**(`genre`·`bpm`·`sort_order` + **점수**) | ★지수 본체 — ★**이 하나가 4개를 막는다**(아래) |
