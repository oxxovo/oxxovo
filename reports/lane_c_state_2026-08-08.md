...
★**필요한 것은 효과가 작용한 곳에서 재는 지표다** — 예: 픽셀별 `ffmpeg delta` 대비
`GL delta`(효과의 자기 크기로 정규화). 그러면 희석이 사라지고 sharpen의 계통 오차가
바닥 위로 올라온다.
★**이건 새 결정(계측 형태)이라 제니2 것이다.** 내가 지표를 바꾸면 표 전체의 의미가 바뀐다.
★그리고 이 결론은 **chromatic·motionBlur에도 그대로 적용된다**(둘 다 국소·저크기).

★**미확인**: `test:parity:transition`·`test:text-parity`·워커 `music-parity`에는 같은 질문을
**아직 안 했다.** 전환 표의 "9종 worst ≤0.30%"도 같은 계열일 수 있다(A 소관).
