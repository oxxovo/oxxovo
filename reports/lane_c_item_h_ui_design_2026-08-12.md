# H — 편집기 UI 설계 (③) — 2026-08-12

★★구현 완료(2026-08-12, 커밋 `90623c7`) — 아래는 설계 당시 문서, 실제 구현은
`ProComposeEditor.tsx`(토글·`setSegSpeedRamp`·`toggleSpeedRamp`) 참조. 실측:
dev 서버+데모 로그인으로 실제 클립 추가→토글 ON(트랙 렌더, 현재 speed로 2점
시딩 확인)→토글 OFF(시간가중평균으로 정적 speed 복원 확인), 콘솔 에러 0.
드래그로 소스초과 차단 자체는 헤드리스 픽셀 드래그 정밀도 문제로 못 잡았으나,
차단 로직은 Stage 1에서 이미 검증된 `speedRampSourceConsumedMs` 위에 얹은
단순 비교문이라 별도 리스크 낮음. 아래 설계 원문은 변경 없이 보존.
