...
## 2. ② 크론 주기 — ★**확정 = `*/5` + `BATCH_SIZE=2`** (본부 판정 2026-08-07)

★**설정값은 예선과 같은 `*/5 * * * *`, `BATCH_SIZE=2` 다.** 아래는 그 판정에
이르는 근거이고, 내가 처음 권고한 `*/15` 도 안전하다는 기록이다 — **설정할 값은 `*/5`.**
★어느 쪽이든 **`BATCH_SIZE=2` 는 고정**이다(겹침 규칙, 아래).
...
★**`*/5` 로 맞출 이유는 하나다**: 예선과 운영을 통일해 사람이 헷갈리지 않게 하는 것.
비용 차이가 거의 없으므로(§6) 그 값이면 그것으로 충분하다.
→ ★**본부 판정 = `*/5` 통일.** 둘 다 안전하고, 운영 통일이 기동 횟수보다 값이 크다는 판단이다.
`*/5` 에서 50편 소진은 **~2.1시간**(2편/5분)이다.
...
★따라서 **`BATCH_SIZE` 는 어느 주기든 `2` 로 고정**한다. 확정 조합
**`BATCH_SIZE=2` + `*/5`** 는 한 실행 ~5분으로 주기와 같아 아슬해 보이지만,
**예선이 이미 그 조합으로 라이브에서 겹치지 않고 돌고 있다**(실측 근거).
본선은 편수가 1/10 이라 큐가 대부분 비어 있고, 만에 하나 겹쳐도 §위의
`in_progress` 제외 + UNIQUE 가 행을 지킨다.
...
→ 본선도 **`2`** 로 간다(본부 확정). 예선이 나중에 30 + `0 * * * *` 로 바뀌어도
본선은 영향받지 않는다 — 서비스가 다르고 변수도 따로 산다.
...
5. **Settings → Cron Schedule** → ★**`*/5 * * * *`** (예선과 동일 — 본부 판정)
6. **Deploy**
7. 첫 실행 로그 확인 (§5)
...
- 11/13 이전(설정~투표 시작): 매 실행 수 초 × `*/5` → **월 몇 분 수준. 무시 가능.**
  (`*/15` 였다면 그 1/3 — 애초에 무시 가능한 값이라 판정에 영향이 없다)
