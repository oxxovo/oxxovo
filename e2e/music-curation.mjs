...
main()
  ...
  .then(() => {
    ...
    console.log("NOT covered: the score ordering. [2.5]'s screen EXISTS (worker src/music-screen.ts,")
    console.log('             fb42108) but there is no column to persist its score, so nothing supplies')
    console.log('             one here. The rule itself is fixed by lib/music-curation-order.test.ts.')
