...
const [aPng, bPng, TYPE = 'fade'] = process.argv.slice(2)
// GL body producing gl_FragColor from a(=uv A), b(=uv B), t(=progress 0.5). Must
...
await run(['-y', '-loop', '1', '-t', '2', '-i', aPng, '-loop', '1', '-t', '2', '-i', bPng,
  '-filter_complex', `[0:v][1:v]xfade=transition=${FFMAP[TYPE]}:duration=1:offset=1,format=rgb24[v]`,
...
const glUrl = await page.evaluate(async ({ aB64, bB64, glBody }) => {
...
const mean = sum / n
console.log(`${TYPE} mid-frame (progress 0.5)`)
